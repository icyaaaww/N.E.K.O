...
LONG_RESPONSE_TAIL_SUMMARY_PROMPT = {
    ...
    'zh-TW': {
        'system': (
            "下面給你同一段話的兩個部分：前半段是已經說出口的，後半段是本來"
            "還要繼續說的。這一輪的話偏長，需要把後半段壓縮成 1-2 句話，"
            "當成這一輪真正的收尾講出來——不是講到一半的總結，是把這一輪的話講完。\n"
            "規則：\n"
            "- 用 1 到 2 句話（不超過 30 個字）自然銜接前半段，把後半段的"
            "核心意思一口氣講完。\n"
            "- 只輸出收尾那段話本身，不要重複前半段，不要加引號。\n"
            "- 保持前半段原有的語氣、口吻和情緒，不要改寫成書面語或換一種腔調。\n"
            "- 不要寫「總結一下」「簡而言之」這類的評語，也不要承認這是摘要。\n"
            "- 聽起來要像把話自然講完，讓聽的人聽不出中間被截斷過。\n"
            "- 這是這一輪的最後一句話，必須有「講完了」的收尾感。可以用"
            "「就這樣吧」「好啦」「差不多」「反正」這類帶收尾意味的銜接詞；"
            "禁止用「還有」「另外」「接下來」「然後」「再講一點」這種"
            "暗示後面還有要講的開頭。"
        ),
        'user_template': (
            "【已經說出口的前半段】\n"
